import gym
import numpy as np
# from gym.spaces import Discrete, Tuple
from ray.rllib.env import MultiAgentEnv

class GridSocialDilemmaEnv(MultiAgentEnv): 
    NAME = 'GridGame'
    NUM_AGENTS = 2
    NUM_ACTIONS = 2

    def __init__(self, max_steps=1, grid_size=2, k=2, **kwargs):
        self.max_steps = max_steps
        self.grid_size = grid_size
        self.num_agents = 2
        self.num_actions = 2
        self.conflict_level = k

        self.initial_position_agent_0 = np.identity(self.grid_size)[0]
        self.initial_position_agent_1 = np.identity(self.grid_size)[-1]
        self.initial_state = np.concatenate([self.initial_position_agent_0, self.initial_position_agent_1],axis=0).squeeze()
        self.agent_positions = [0, self.grid_size - 1]
        
        self.action_space = gym.spaces.Discrete(2)
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(grid_size*self.num_agents,),dtype=np.float32)  # alternative dummy observation [1, 0]
        self.num_outputs = 2
        self.metrics = {'transfers':0} # initial transfer is 0

    def reset(self):
        self.current_step = 0
        self.agent_positions = [0, self.grid_size - 1]
        state = self.initial_state
        return {
            "a0": state,
            "a1": state
        }

    def step(self, actions):
        self.current_step += 1

        # ac0 = actions["a0"]
        # ac1 = actions["a1"]
        # print("actions", actions)
        
        for i in range(self.num_agents):
            agent_idx = "a" + str(i)
            if actions[agent_idx]==0: # stay
                if self.agent_positions[i] > 0:
                    self.agent_positions[i] -= 1
                elif self.agent_positions[i]==0:
                    self.agent_positions[i] = 0
                else:
                    AssertionError("Negative position")
            elif actions[agent_idx]==1:
                if self.agent_positions[i] < self.grid_size - 1:
                    self.agent_positions[i] += 1
                elif self.agent_positions[i]==self.grid_size-1:
                    self.agent_positions[i] = self.grid_size - 1
                else:
                    AssertionError("Over the grid size")
        
        # reveal agents' positions
        state = np.concatenate([np.identity(self.grid_size)[self.agent_positions[0]], np.identity(self.grid_size)[self.agent_positions[1]]],axis=0).squeeze()      
        # print("state", state)
        immediate_rewards = self.calculate_rewards()
        rewards = {
            "a0": immediate_rewards[0],
            "a1": immediate_rewards[1]
        }
        observations = {
            "a0": state,
            "a1": state
        }
        done = (self.current_step == self.max_steps)
        dones = {"__all__": done}

        infos = {
            "a0": {},
            "a1": {}
        }
        
        return observations, rewards, dones, infos

    def calculate_rewards(self):
        pos_0, pos_1 = self.agent_positions
        reward_0 = pos_0 - self.conflict_level*(self.grid_size-1 - pos_1)
        reward_1 = self.grid_size-1 - pos_1 - self.conflict_level*pos_0
        rewards = [reward_0, reward_1]
        return rewards

    # def render(self):
    #     grid = [' ' for _ in range(self.grid_size)]
    #     if self.agent_positions[0] == self.agent_positions[1]:
    #         grid[self.agent_positions[0]] = 'AB'
    #     else:
    #         grid[self.agent_positions[0]] = 'A'
    #         grid[self.agent_positions[1]] = 'B'
    #     print('|'.join(grid))
    #     print(f"Agent A Position: {self.agent_positions[0]}, Agent B Position: {self.agent_positions[1]}")
